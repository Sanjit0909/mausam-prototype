...
export function HourlyForecast({ hourly, limit = 24 }: HourlyForecastProps) {
  ...
  return (
    <div className="glass rounded-3xl p-6">
      <h3 className="text-sm font-semibold text-mist-200 mb-4">Hourly Forecast</h3>
      <div className="flex gap-4 overflow-x-auto pb-2 -mx-1 px-1">
        {points.map((point, i) => (
          <div key={point.time} className="flex min-w-[64px] flex-col items-center gap-2 text-center">
            <span className="text-xs text-mist-400">{i === 0 ? "Now" : formatHourLabel(point.time)}</span>
            ...
            {point.precipitation_probability !== null && point.precipitation_probability !== undefined && point.precipitation_probability > 0 && (
