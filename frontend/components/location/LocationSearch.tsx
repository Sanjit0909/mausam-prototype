...
export function LocationSearch({ onSelect, autoFocus, placeholder }: LocationSearchProps) {
  ...
  useEffect(() => {
    ...
    searchLocations(debouncedQuery, controller.signal)
      .then(setResults)
      .catch((err) => {
        if (err.name !== "AbortError") setError(t("location.searchError"));
      })
  ...
  return (
    <div className="w-full">
      ...
      >
        ...
        {geoLoading ? t("location.locating") : t("location.useCurrent")}
      </button>
      ...
      {!loading && debouncedQuery.trim().length >= 2 && results.length === 0 && !error && (
        <p className="mt-3 px-1 text-sm text-mist-400">{t("location.noneFound", { query: debouncedQuery })}</p>
      )}
    </div>
  );
}
